...
export default function ExGen() {
    ...
    const [showRestTimerSlider, setShowRestTimerSlider] = useState(false);
    ...
    const progressRestTime = useSharedValue(90);
    const minRestTime = useSharedValue(0);
    const maxRestTime = useSharedValue(180);

    const progressWorkoutTime = useSharedValue(60); 
    const minWorkoutTime = useSharedValue(15); 
    const maxWorkoutTime = useSharedValue(180); 
    ...
    const handleGenerateWorkout = async () => {
        if (!goals || !equipment || !timeInMinutes || !currentFitnessLevel) {
            ...
            if (inWorkoutSection) {
                if (lowerLine.startsWith('exercise name:')) {
                    ...
                    if (lowerLine.startsWith('sets:')) {
                        ...
                        currentReps = line.substring('Reps:'.length).trim() || 'N/A'; 
    ...
    return (
        <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
            ...
            <View style={styles.fitnessLevelButtonsContainer}>
                <TouchableOpacity
                    style={[styles.actionButton, currentFitnessLevel === 'Beginner' && styles.actionButtonActive]}
                    onPress={() => setCurrentFitnessLevel('Beginner')}
                >
                    <Text style={styles.actionButtonText}>Beginner</Text>
                </TouchableOpacity>

                <TouchableOpacity
                    style={[styles.actionButton, currentFitnessLevel === 'Intermediate' && styles.actionButtonActive]}
                    onPress={() => setCurrentFitnessLevel('Intermediate')}
                >
                    <Text style={styles.actionButtonText}>Intermediate</Text>
                </TouchableOpacity>

                <TouchableOpacity
                    style={[styles.actionButton, currentFitnessLevel === 'Advanced' && styles.actionButtonActive]}
                    onPress={() => setCurrentFitnessLevel('Advanced')}
                >
            ...
            <View style={styles.restTimerSection}>
                <TouchableOpacity
                    ...
                    onPress={() => setShowRestTimerSlider(!showRestTimerSlider)}
                >
                    <Text style={styles.restTimerButtonText}>Rest Timer</Text>
                </TouchableOpacity>

                {showRestTimerSlider && (
                    <View style={styles.sliderWrapper}>
                        <Text style={styles.sliderValueText}>{restTime}s</Text>
                        <Slider
                            theme={{
                                minimumTrackTintColor: '#DA9E44',
                                maximumTrackTintColor: '#444', 
                            ...
                            style={styles.slider}
                            progress={progressRestTime} 
                            minimumValue={minRestTime}   
                            maximumValue={maxRestTime}   
                            step={12}
                            snapToStep
                            onValueChange={(value: number) => {
                                setRestTime(Math.round(value));
                                progressRestTime.value = value; 
                            }}
                            renderBubble={() => null} 
                        />
                    </View>
